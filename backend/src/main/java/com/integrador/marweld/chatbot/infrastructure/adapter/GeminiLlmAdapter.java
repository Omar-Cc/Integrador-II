...
/**
 * Adaptador de infraestructura que consume el API de Google AI Gemini (generateContent).
 */
@Component
public class GeminiLlmAdapter implements LlmClientPort {
    ...
    public GeminiLlmAdapter(ChatbotProperties properties) {
    ...
    public LlmResponse generateResponse(LlmPrompt prompt) {
        ...
        if (apiKey == null || apiKey.isBlank()) {
            log.warn("Llamada a Gemini sin API Key configurada. Retornando respuesta de fallback.");
        ...
        try {
            ...
            // Armar el prompt consolidando contexto e historial conversacional
            String fullPrompt = buildSystemPrompt(prompt) + "\n\n" +
                    buildConversationAndUserQuery(prompt);

            // Construir payload según la estructura de Gemini API
            Map<String, Object> textPart = Map.of("text", fullPrompt);
            Map<String, Object> contentObj = Map.of("parts", List.of(textPart));
            
            Map<String, Object> generationConfig = Map.of(
                    "temperature", properties.getLlm().getGemini().getTemperature()
            );
            ...
            log.info("Invocando Gemini API con modelo: {}", model);

            Map<String, Object> response = restTemplate.postForObject(url, entity, Map.class);
            if (response == null || !response.containsKey("candidates")) {
                throw new IllegalStateException("Respuesta incorrecta de Gemini API");
            }
            ...
            Integer promptTokens = usageMetadata != null ? ((Number) usageMetadata.get("promptTokenCount")).intValue() : 0;
            Integer completionTokens = usageMetadata != null ? ((Number) usageMetadata.get("candidatesTokenCount")).intValue() : 0;
            ...
            log.error("Error al invocar Gemini API: {}", e.getMessage(), e);
            throw new RuntimeException("Error en la comunicación con Gemini: " + e.getMessage(), e);
        }
    }
    ...
    private String buildConversationAndUserQuery(LlmPrompt prompt) {
        StringBuilder sb = new StringBuilder("--- HISTORIAL DE INTERACCIÓN Y ENTRADA ---\n");
        prompt.conversationHistory().forEach(msg -> {
            sb.append(msg.getEmisor()).append(": ").append(msg.getContenido()).append("\n");
        });
        sb.append("USUARIO: ").append(prompt.userMessage()).append("\n");
        sb.append("BOT:");
        return sb.toString();
    }
